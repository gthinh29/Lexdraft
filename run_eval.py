...
def main():
    ...
    print("\n" + "=" * 65)
    print("CHI SO DANH GIA CHUAN RAGAS (RAG ASSESSMENT METRICS):")
    print(
        f"   - Tong so cau hoi kiem thu:                         {metrics.get('total_samples', 0)}"
    )
    print(
        f"   1. Faithfulness (Do trung thuc / Chong ao giac):    {metrics.get('faithfulness', 0.0):.4f} ({metrics.get('faithfulness', 0.0) * 100:.1f}%)"
    )
    print(
        f"   2. Answer Relevancy (Do lien quan / Trong tam):     {metrics.get('answer_relevancy', 0.0):.4f} ({metrics.get('answer_relevancy', 0.0) * 100:.1f}%)"
    )
    print(
        f"   3. Context Precision (Do chinh xac ngu canh FAISS): {metrics.get('context_precision', 0.0):.4f} ({metrics.get('context_precision', 0.0) * 100:.1f}%)"
    )
    print(
        f"   4. Context Recall (Do bao phu can cu phap ly):      {metrics.get('context_recall', 0.0):.4f} ({metrics.get('context_recall', 0.0) * 100:.1f}%)"
    )
    print("=" * 65)
